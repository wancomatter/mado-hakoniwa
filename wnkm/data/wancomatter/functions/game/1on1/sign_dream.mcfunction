#
#subcounter2 -> ステージ番号(遺跡=2)
#
#

execute unless entity @s[team=z_spectate] if entity @e[tag=1on1_entry,limit=1] as @e[tag=1on1_entry] if score @s subcounter2 matches 2 run tag @s add flag
...
scoreboard players set @e[tag=this_1on1] subcounter2 2
